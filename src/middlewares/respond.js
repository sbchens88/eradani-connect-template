    ...
    return (req, res, next) => {
        ...
        return response
            ...
            .catch(error => {
                _filterProtectedFields(req);
                if (error.status === 500) {
                    logger.error({ message: error.message, status: error.status, additionalData: { rawError: _stringifyError(error), route: req.originalUrl.split('?')[0], data: { query: Object.keys(req.query).length ? req.query : "No Query Data", params: Object.keys(req.params).length ? req.params : "No URL Param Data", body: Object.keys(req.body).length ? req.body : "No Body Data" }, user: !req.user || !Object.keys(req.user).length ? "Unidentified User" : req.user } });
                } else {
                    logger.warn({ message: error.message, status: error.status, additionalData: { rawError: _stringifyError(error), route: req.originalUrl.split('?')[0], data: { query: Object.keys(req.query).length ? req.query : "No Query Data", params: Object.keys(req.params).length ? req.params : "No URL Param Data", body: Object.keys(req.body).length ? req.body : "No Body Data" }, user: !req.user || !Object.keys(req.user).length ? "Unidentified User" : req.user } });
                }
            ...
            .catch(error => {
                logger.error({ message: error.message || "Unknown Error", status: error.status || error.statusCode || 500, additionalData: { rawError: _stringifyError(error), route: req.originalUrl.split('?')[0], data: { query: Object.keys(req.query).length ? req.query : "No Query Data", params: Object.keys(req.params).length ? req.params : "No URL Param Data", body: Object.keys(req.body).length ? req.body : "No Body Data" }, user: !req.user || !Object.keys(req.user).length ? "Unidentified User" : req.user } });
